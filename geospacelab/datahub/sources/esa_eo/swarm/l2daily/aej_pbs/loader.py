...
default_variable_name_dict = {
    ...
    'QD_MLT_PB': 'MLT_PB',
    'QUALITY_FLAG': 'Flags',
    'GEO_LAT_B': 'Latitude_B',
    'GEO_LON_B': 'Longitude_B',
    'B': 'B',
}
...
class Loader(LoaderModel):
    ...
    def load_data(self, **kwargs):
        ...
        self.variables['WEJ_PEAK'] = self.variables['J_DF_PEAK'][:, 0][:, np.newaxis]
        ...
        self.variables['EEJ_PEAK'] = self.variables['J_DF_PEAK'][:, 1][:, np.newaxis]
        ...
        self.variables['B_N_MAX'] = self.variables['B'][:, 0, 0][:, np.newaxis]
        self.variables['B_N_MIN'] = self.variables['B'][:, 1, 0][:, np.newaxis]
        self.variables['B_E_MAX'] = self.variables['B'][:, 0, 1][:, np.newaxis]
        self.variables['B_E_MIN'] = self.variables['B'][:, 1, 1][:, np.newaxis]
        self.variables['GEO_LAT_B_MIN'] = self.variables['GEO_LAT_B'][:, 0][:, np.newaxis]
        self.variables['GEO_LON_B_MIN'] = self.variables['GEO_LON_B'][:, 0][:, np.newaxis]
