...
class Downloader(DownloaderSwarm):
    ...
    def __init__(
        ...
        if data_file_root_dir is None:
            data_file_root_dir = prf.datahub_data_root_dir / "ESA" / "SWARM" / "Level2daily" / "EFI_TIE"
        root_dir_local = data_file_root_dir
        
        self._default_sub_dirs_remote = {
            'latest': ['Latest_baselines', 'EFI', 'TIE'],
            # 'Older_baselines': ['Older_baselines', 'EFI_TIE'], 
            'Entire_mission': ['Entire_mission_data', 'EFI', 'TIE']
        }
