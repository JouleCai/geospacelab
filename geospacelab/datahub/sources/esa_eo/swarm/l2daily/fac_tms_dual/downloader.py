...
class Downloader(DownloaderSwarm):
    ...
    def __init__(
        ...
        file_class = 'OPER'
        product = 'FAC_TMS'
        sat_id = 'AC' # The product is only for AC, but the file name does not contain 'AC', so we set sat_id to 'AC' but do not use it for file searching.
        product_level = '2F'
        root_dir_remote = '/Level2daily'
        
        if data_file_root_dir is None:
            data_file_root_dir = prf.datahub_data_root_dir / "ESA" / "SWARM" / "Level2daily" / "FAC_TMS_DUAL"
        root_dir_local = data_file_root_dir
        
        self._default_sub_dirs_remote = {
            'latest': ['Latest_baselines', 'FAC', 'TMS'],
            'Older_baselines': ['Older_baselines', 'FAC', 'TMS'], 
            'Entire_mission': ['Entire_mission_data', 'FAC', 'TMS']
        }
