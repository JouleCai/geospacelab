...
def test_ampere_new():
    ...
    dt_to = datetime.datetime(2024, 5, 10, 23, 59)
    time_c = datetime.datetime(2024, 5, 10, 18, 0)
    pole = 'N'
    
    db = geomap.GeoDashboard(dt_fr=dt_fr, dt_to=dt_to, figure_config={'figsize': (8, 8)})
    # NOTE: Since 2026, the 'deprecated' data source has been moved to a separate directory under 'jhuapl/ampere/deprecated'. So the datasource_contents should be updated to ['jhuapl', 'ampere', 'deprecated', 'grd'].
    db.dock(datasource_contents=['jhuapl', 'ampere', 'grd'], pole=pole)
    db.set_layout(1, 1)
    ds_ampere = db.datasets[0]

    fac = ds_ampere['GRID_j_FA']  # or db.assign_variable('GRID_j_FA', dataset_index=0)
    ...
    ind_t = ds_ampere.get_time_ind(ut=time_c)
    # initialize the polar map
    panel = db.add_polar_map(row_ind=0, col_ind=0, style='mlt-fixed', cs='AACGM', mlt_c=0., pole=pole, ut=time_c, boundary_lat=40, mirror_south=True)
    ...
    grid_mlat, grid_mlt, grid_fac = ds_ampere.grid_fac(fac_, mlt_res=1., mlat_res=1., interp_method='linear')
    ...
    pcolormesh_config.update(c_lim=[-3, 3])
    pcolormesh_config.update(shading='auto')
    import geospacelab.visualization.mpl.colormaps as cm
    cmap = cm.cmap_gist_ncar_modified()
    cmap = 'jet'
    pcolormesh_config.update(cmap=cmap)
    # ipc = panel.add_pcolor(fac_, coords={'lat': mlat[ind_t, ::], 'lon': None, 'mlt': mlt[ind_t, ::], 'height': 250.}, cs='AACGM', **pcolormesh_config)
    ipm = panel.overlay_pcolormesh(grid_fac, coords={'lat': grid_mlat, 'lon': None, 'mlt': grid_mlt, 'height': 780.}, cs='AACGM', **pcolormesh_config, regridding=False)
    panel.add_colorbar(ipm, c_label=r'FAC ($\mu$A/m$^2$)', c_scale=pcolormesh_config['c_scale'], left=1.1, bottom=0.1,
...
def test_ampere_deprecated():
    dt_fr = datetime.datetime(2024, 5, 8, 0)
    dt_to = datetime.datetime(2024, 5, 13, 23, 59)
    time_c = datetime.datetime(2024, 5, 8, 10, 10)
    pole = 'N'
    
    db = geomap.GeoDashboard(dt_fr=dt_fr, dt_to=dt_to, figure_config={'figsize': (8, 8)})
    # NOTE: Since 2026, the 'deprecated' data source has been moved to a separate directory under 'jhuapl/ampere/deprecated'. So the datasource_contents should be updated to ['jhuapl', 'ampere', 'deprecated', 'grd'].
    db.dock(datasource_contents=['jhuapl', 'ampere', 'deprecated','grd'], pole=pole)
    db.set_layout(1, 1)
    ds_ampere = db.datasets[0]
    ...
    ind_t = ds_ampere.get_time_ind(ut=time_c)
    # initialize the polar map
    panel = db.add_polar_map(row_ind=0, col_ind=0, style='mlt-fixed', cs='AACGM', mlt_c=0., pole=pole, ut=time_c, boundary_lat=60, mirror_south=True)
