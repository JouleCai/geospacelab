...
def show_dmsp(sat_id=None, pole='N', band='LBHS', dt_c=None, file_dir=None):
    ...
    # Overlay sites
    panel1.overlay_sites(site_ids=['TRO', 'ESR'], coords={'lat': [69.58, 78.15], 'lon': [19.23, 16.02], 'height': 1.},
                         cs='GEO', marker='^', markersize=3)
...
def search_record(dt_fr, dt_to, sat_id, center='N'):
    # check outliers
    orbit_obj = OrbitPosition_SSCWS(
        dt_fr=dt_fr,
        dt_to=dt_to,
        sat_id='dmsp' + sat_id.lower()
    )

    lats = orbit_obj['SC_GEO_LAT'].value.flatten()
    lons = orbit_obj['SC_GEO_LON'].value.flatten()
    alts = orbit_obj['SC_GEO_ALT'].value.flatten()
    sc_dts = orbit_obj['SC_DATETIME'].value.flatten()

    sat_ids = []
    dts = []

    if center == 'N':
        ind_center = argrelextrema(lats, np.greater)[0]
        ind_center = ind_center[np.where(lats[ind_center] > 60)[0]]

    if center == 'S':
        ind_center = argrelextrema(lats, np.less)[0]
        ind_center = ind_center[np.where(lats[ind_center] < -60)[0]]

    for ind in ind_center:
        dt = sc_dts[ind]
        dts.append(dt)
...
def storm_events():
    ...
    dt_to = datetime.datetime(2016, 3, 22, 0)

    sat_ids = ['f16', 'f17', 'f18']
    falsed_passes = []
    file_dir = pathlib.Path(__file__).parent.resolve() / "results"
    for sat_id in sat_ids:
        dts = search_record(dt_fr, dt_to, sat_id=sat_id)

        for dt in dts:
            # try:
            #    show_dmsp(sat_id=sat_id, dt_c=dt, file_dir=file_dir)
            # except:
            #    falsed_passes.append([dt, sat_id])
            show_dmsp(sat_id=sat_id, dt_c=dt, file_dir=file_dir)

    for info in falsed_passes:
        print(info)
